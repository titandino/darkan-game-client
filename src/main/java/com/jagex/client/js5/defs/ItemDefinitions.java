...
import com.jagex.unknown.Class304;
import com.jagex.unknown.Class329;
...
public class ItemDefinitions implements Definition {
	...
	public static int[] addOutlines(int[] ints_0, int i_1) {
	...
	public static void method7140(PathingEntity animable_0, PathingEntity animable_1, int i_2, int i_3, int i_4) {
		...
		if (i_10 != -1) {
			...
			HitsplatIndexLoader.method3612(animable_0.plane, (int) vector3_16.x, (int) vector3_16.z, animable_0.getSize() * 256, 0, false);
	...
	public MeshRasterizer method7084(AbstractRenderer renderer, int i_2, int i_3, PlayerModel playerappearance_4, Animation animation_5, int i_6, int i_7, int i_8, int i_9) {
		int i_11;
		if (stackIds != null && i_3 > 1) {
			...
			if (i_11 != -1) {
				return loader.getItemDefinitions(i_11).method7084(renderer, i_2, 1, playerappearance_4, animation_5, i_6, i_7, i_8, i_9);
			}
		}
		i_11 = i_2;
		if (animation_5 != null) {
		...
		MeshRasterizer inventoryModelMeshRasterizer;
		synchronized (loader.aClass229_5115) {
			inventoryModelMeshRasterizer = (MeshRasterizer) loader.aClass229_5115.get(id | renderer.rendererId << 29);
		}
		if (inventoryModelMeshRasterizer == null || renderer.method8452(inventoryModelMeshRasterizer.m(), i_11) != 0) {
			if (inventoryModelMeshRasterizer != null) {
				i_11 = renderer.method8546(i_11, inventoryModelMeshRasterizer.m());
			}
			int i_18 = i_11;
			...
			RSMesh inventoryModelMesh = RSMesh.decodeMesh(loader.meshIndex, modelId);
			if (inventoryModelMesh == null) {
				return null;
			}
			if (inventoryModelMesh.version < 13) {
				inventoryModelMesh.upscale();
			}
			inventoryModelMeshRasterizer = renderer.createMeshRasterizer(inventoryModelMesh, i_18, loader.anInt5116, ambient + 64, contrast * 5 + 850);
			if (resizeX != 128 || resizeY != 128 || resizeZ != 128) {
				inventoryModelMeshRasterizer.resize(resizeX, resizeY, resizeZ);
			}
			int i_15;
			if (originalModelColors != null) {
				for (i_15 = 0; i_15 < originalModelColors.length; i_15++) {
					if (spriteRecolorIndices != null && i_15 < spriteRecolorIndices.length) {
						inventoryModelMeshRasterizer.recolour(originalModelColors[i_15], staticColorArr256[spriteRecolorIndices[i_15] & 0xff]);
					} else {
						inventoryModelMeshRasterizer.recolour(originalModelColors[i_15], modifiedModelColors[i_15]);
					}
				}
			}
			if (originalTextureIds != null) {
				for (i_15 = 0; i_15 < originalTextureIds.length; i_15++) {
					inventoryModelMeshRasterizer.retexture(originalTextureIds[i_15], modifiedTextureIds[i_15]);
				}
			}
			if (playerappearance_4 != null) {
				for (i_15 = 0; i_15 < 10; i_15++) {
					for (int i_16 = 0; i_16 < AbstractQueue_Sub1.DEFAULT_SKIN_COLOURS[i_15].length; i_16++) {
						if (playerappearance_4.colours[i_15] < Class366.SKIN_COLORS[i_15][i_16].length) {
							inventoryModelMeshRasterizer.recolour(AbstractQueue_Sub1.DEFAULT_SKIN_COLOURS[i_15][i_16], Class366.SKIN_COLORS[i_15][i_16][playerappearance_4.colours[i_15]]);
						}
					}
				}
			}
			inventoryModelMeshRasterizer.KA(i_11);
			LRUCache softcache_19 = loader.aClass229_5115;
			synchronized (loader.aClass229_5115) {
				loader.aClass229_5115.put(inventoryModelMeshRasterizer, id | renderer.rendererId << 29);
			}
		}
		if (animation_5 != null || i_9 != 0) {
			inventoryModelMeshRasterizer = inventoryModelMeshRasterizer.method11289((byte) 1, i_11, true);
			if (animation_5 != null) {
				animation_5.rasterize(inventoryModelMeshRasterizer, 0);
			}
			if (i_9 != 0) {
				inventoryModelMeshRasterizer.PA(i_6, i_7, i_8, i_9);
			}
		}
		inventoryModelMeshRasterizer.KA(i_2);
		return inventoryModelMeshRasterizer;
	}
	...
	public void generateBind(ItemDefinitions itemdefinitions_1, ItemDefinitions itemdefinitions_2) {
		value = 0;
	...
	public void generateCert(ItemDefinitions itemdefinitions_1, ItemDefinitions itemdefinitions_2) {
		modelId = itemdefinitions_1.modelId;
		modelZoom = itemdefinitions_1.modelZoom;
		modelRotationX = itemdefinitions_1.modelRotationX;
		modelRotationY = itemdefinitions_1.modelRotationY;
	...
	public void decode(ByteBuf stream, int opcode) {
		if (opcode == 1) {
	...
	public int[] getSprite(AbstractRenderer hardwareRenderer, AbstractRenderer softwareRenderer, int amount, int outlineSize, int shadowColor, boolean zoomedIn, int renderAmounts, FontRenderer fontRenderer, PlayerModel model) {
		...
		if (inventoryMesh == null) {
			...
			if (model != null) {
				for (i = 0; i < 10; i++) {
					for (int i_13 = 0; i_13 < AbstractQueue_Sub1.DEFAULT_SKIN_COLOURS[i].length; i_13++) {
						if (model.colours[i] < Class366.SKIN_COLORS[i][i_13].length) {
							inventoryMesh.recolor(AbstractQueue_Sub1.DEFAULT_SKIN_COLOURS[i][i_13], Class366.SKIN_COLORS[i][i_13][model.colours[i]]);
						}
					}
				}
			}
			...
			if (!meshRasterizer.successful()) {
				...
				if (needsResize) {
					meshRasterizer.resize(resizeX, resizeY, resizeZ);
				}
	...
	public void addShadow(int[] ints_1, int i_2) {
	...
	public RSMesh getBodyMesh(boolean isFemale, MeshModifier modifier) {
		...
		if (equip1 == -1)
			return null;
		RSMesh mesh = RSMesh.decodeMesh(loader.meshIndex, equip1);
		if (mesh == null)
			return null;
		if (mesh.version < 13)
			mesh.upscale();
		if (equip2 != -1) {
			RSMesh equip2Mesh = RSMesh.decodeMesh(loader.meshIndex, equip2);
			if (equip2Mesh.version < 13)
				equip2Mesh.upscale();
			if (equip3 != -1) {
				RSMesh equipMesh3 = RSMesh.decodeMesh(loader.meshIndex, equip3);
				if (equipMesh3.version < 13)
					equipMesh3.upscale();
				RSMesh[] meshes = {mesh, equip2Mesh, equipMesh3};
		...
		short[] modified;
		if (originalModelColors != null) {
			if (modifier != null && modifier.modifiedColors != null)
				modified = modifier.modifiedColors;
			else
				modified = modifiedModelColors;
			for (i = 0; i < originalModelColors.length; i++)
				mesh.recolor(originalModelColors[i], modified[i]);
		}
		if (originalTextureIds != null) {
	...
	public boolean headMeshesReady(boolean bool_1, MeshModifier meshmodifier_2) {
		...
		int i_5;
		if (bool_1) {
			if (meshmodifier_2 != null && meshmodifier_2.femaleHead != null) {
				i_4 = meshmodifier_2.femaleHead[0];
				i_5 = meshmodifier_2.femaleHead[1];
			} else {
				i_4 = femaleHead1;
				i_5 = femaleHead2;
			}
		} else if (meshmodifier_2 != null && meshmodifier_2.maleHead != null) {
			i_4 = meshmodifier_2.maleHead[0];
			i_5 = meshmodifier_2.maleHead[1];
		} else {
			i_4 = maleHead1;
			i_5 = maleHead2;
		}
	...
	public RSMesh getHeadMesh(boolean isFemale, MeshModifier modifier) {
		...
		if (headModel1 == -1)
			return null;
		RSMesh mesh = RSMesh.decodeMesh(loader.meshIndex, headModel1);
		if (mesh.version < 13)
			mesh.upscale();
		if (headModel2 != -1) {
			RSMesh mesh2 = RSMesh.decodeMesh(loader.meshIndex, headModel2);
			if (mesh2.version < 13)
				mesh2.upscale();
	...
	public void generateLend(ItemDefinitions itemdefinitions_1, ItemDefinitions itemdefinitions_2) {
		value = 0;
		modelId = itemdefinitions_1.modelId;
		modelZoom = itemdefinitions_1.modelZoom;
		modelRotationX = itemdefinitions_1.modelRotationX;
