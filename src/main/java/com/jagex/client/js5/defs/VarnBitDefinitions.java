package com.jagex.client.js5.defs;

import com.jagex.Node_Sub31;
import com.jagex.net.io.ByteBuf;
import com.jagex.unknown.Class476;

public class VarnBitDefinitions {

	public static int anInt5765;
	public int anInt5764;
	public int anInt5763;
	public int anInt5762;

	public static void method8217(int i_0, int i_1, int i_2, int i_3, int i_4, int i_5, Class476 class476_6) {
		Node_Sub31 class282_sub31_8 = null;
		for (Node_Sub31 class282_sub31_9 = (Node_Sub31) Node_Sub31.aClass482_7776.head(); class282_sub31_9 != null; class282_sub31_9 = (Node_Sub31) Node_Sub31.aClass482_7776.next()) {
			if (i_0 == class282_sub31_9.anInt7764 && class282_sub31_9.anInt7762 == i_1 && i_2 == class282_sub31_9.anInt7763 && i_3 == class282_sub31_9.anInt7766) {
				class282_sub31_8 = class282_sub31_9;
				break;
			}
		}
		if (class282_sub31_8 == null) {
			class282_sub31_8 = new Node_Sub31();
			class282_sub31_8.anInt7764 = i_0;
			class282_sub31_8.anInt7766 = i_3;
			class282_sub31_8.anInt7762 = i_1;
			class282_sub31_8.anInt7763 = i_2;
			Node_Sub31.aClass482_7776.append(class282_sub31_8);
		}
		class282_sub31_8.anInt7769 = i_4;
		class282_sub31_8.anInt7771 = i_5;
		class282_sub31_8.aClass476_7768 = class476_6;
		class282_sub31_8.aBool7773 = true;
		class282_sub31_8.aBool7774 = false;
	}

	public void method8213(ByteBuf rsbytebuffer_1) {
		while (true) {
			int i_3 = rsbytebuffer_1.readUnsignedByte();
			if (i_3 == 0) {
				return;
			}
			method8214(rsbytebuffer_1, i_3);
		}
	}

	public void method8214(ByteBuf rsbytebuffer_1, int i_2) {
		if (i_2 == 1) {
			anInt5764 = rsbytebuffer_1.readUnsignedShort();
			anInt5763 = rsbytebuffer_1.readUnsignedByte();
			anInt5762 = rsbytebuffer_1.readUnsignedByte();
		}
	}
}
