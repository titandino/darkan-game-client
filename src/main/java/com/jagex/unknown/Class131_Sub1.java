package com.jagex.unknown;

import com.jagex.Interface14;
import com.jagex.client.render.opengl.OpenGLRenderer;
import jaclib.memory.Buffer;

public class Class131_Sub1 extends Class131 implements Interface14 {

	public int anInt9016;

	public Class131_Sub1(OpenGLRenderer class505_sub1_1, int i_2, byte[] bytes_3, int i_4) {
		super(class505_sub1_1, bytes_3, i_4);
		anInt9016 = i_2;
	}

	public Class131_Sub1(OpenGLRenderer class505_sub1_1, int i_2, Buffer buffer_3) {
		super(class505_sub1_1, buffer_3);
		anInt9016 = i_2;
	}

	@Override
	public int method3() {
		return 0;
	}

	@Override
	public void method107(int i_1, byte[] bytes_2, int i_3) {
		method2319(bytes_2, i_3);
		anInt9016 = i_1;
	}

	@Override
	public int method1() {
		return 0;
	}

	@Override
	public long method109() {
		return aBuffer1593.method2();
	}

	@Override
	public void method106(int i_1, byte[] bytes_2, int i_3) {
		method2319(bytes_2, i_3);
		anInt9016 = i_1;
	}

	@Override
	public int method4() {
		return 0;
	}

	@Override
	public int method75() {
		return anInt9016;
	}

	@Override
	public long method6() {
		return aBuffer1593.method2();
	}

	@Override
	public int method59() {
		return anInt9016;
	}

	@Override
	public void method108(int i_1, byte[] bytes_2, int i_3) {
		method2319(bytes_2, i_3);
		anInt9016 = i_1;
	}

}
