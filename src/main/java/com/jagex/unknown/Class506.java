...
public class Class506 {
	...
	public static int USE_OPTIONS_FLAGS;

	public static Class223 CS2_CACHE = new Class223(128);
	...
	public static MeshModifier decodeItemEffects(ItemDefinitions defs, ByteBuf stream) {
		...
		if (headModels) {
			meshModifier.maleHead[0] = stream.readBigSmart();
			meshModifier.femaleHead[0] = stream.readBigSmart();
			if (defs.maleHead2 != -1 || defs.femaleHead2 != -1) {
				meshModifier.maleHead[1] = stream.readBigSmart();
				meshModifier.femaleHead[1] = stream.readBigSmart();
			}
		}
		if (colors) {
