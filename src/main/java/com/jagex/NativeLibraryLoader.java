...
import com.jagex.unknown.OperatingSystemInfo;
...
public class NativeLibraryLoader implements LibraryLoader {
	...
	public String OS_AND_ARCHITECTURE;

	public NativeLibraryLoader(Index nativeIndex) {
		nativeLibraryIndex = nativeIndex;
		String str_2 = "";
		if (!OperatingSystemInfo.OS_NAME.startsWith("win") && !OperatingSystemInfo.OS_NAME.startsWith("windows 7")) {
			if (OperatingSystemInfo.OS_NAME.startsWith("linux")) {
				str_2 += "linux/";
			} else if (OperatingSystemInfo.OS_NAME.startsWith("mac")) {
				str_2 += "macos/";
			}
		} else {
			str_2 += "windows/";
		}
		if (!OperatingSystemInfo.OS_ARCHITECTURE.startsWith("amd64") && !OperatingSystemInfo.OS_ARCHITECTURE.startsWith("x86_64")) {
			if (!OperatingSystemInfo.OS_ARCHITECTURE.startsWith("i386") && !OperatingSystemInfo.OS_ARCHITECTURE.startsWith("i486") && !OperatingSystemInfo.OS_ARCHITECTURE.startsWith("i586") && !OperatingSystemInfo.OS_ARCHITECTURE.startsWith("x86")) {
				if (OperatingSystemInfo.OS_ARCHITECTURE.startsWith("ppc")) {
					str_2 += "ppc/";
				} else {
					str_2 += "universal/";
				}
	...
	public void method4637(String string_1, File file_2) {
